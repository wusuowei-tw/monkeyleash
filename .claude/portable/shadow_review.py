...
def promotion_status(path):
    """回傳每條規則的計數與晉升判定。**這是報表,不是閘門。**

    ## ⚠ 先說清楚它不做什麼(2026-08-26 追蹤,可複驗)

    本函式算出的 `promotable`,**全庫只有一個讀取點**:
    `print_status` 拿它選「可轉正」或「留影子」兩個字串。
    `main()` 只有 `ShadowLogError` 回 1;`gate.py` **沒有 import 本模組**。

    更深一層:`shadow_active()` 是 **per-repo 的布林值**,`gate.py` 的三個
    呼叫點全是裸的 `if shadow_active():`,**沒有規則參數** ——
    所以 ADR 0012 的「晉升 per-rule,不全局」**未實作**,
    而**「晉升」這個動作本身也不存在**:實際會發生的只有到期。

    **裁決(2026-08-26):保留為報表,不補實作成閘門。** 三條理由 ——
    (a) 三個門檻值沒有出處(`F-124`),把沒有理由的數字變成閘門
    等於把猜測升格成規則;(b) 補 per-rule 晉升是一大塊新工作,
    12 天窗口內不該開;(c) 全局 enforce 已有理由相信可工作(見票 67)。

    **這段不是免責聲明,是防再誤讀的機制** ——
    報表與閘門在文件裡長得一模一樣,兩者都用「條件…才可…」的句式。
    釘子在 `TestFTheThresholdsAnnounceThatTheyAreNotAGate`。
