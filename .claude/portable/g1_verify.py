...
#
# ⚠ **出這一桶的門檻(裁決 2026-08-31,票 92 AC-3):**
#   **repo 內補好不算,要等那一票的 AC-1 勾了才移** ——
#   即 Jeff 完成 `~/.claude/hooks/g1_guard.py` 的人工覆蓋(ADR 0009)、
#   並且本工具全綠的那一刻。
#
#   理由:此刻移掉,末行會宣稱一個「在這台機器上還沒成立」的事實。
#   那個數字現在往**悲觀**方向說謊(算著一個已補的缺口),
#   移得太早會變成往**樂觀**說謊 —— **兩種都不準,但後者更貴。**
#
#   **進的門檻寫過(上一段),而出的門檻在 2026-08-31 之前沒有人寫過** ——
#   而**一個只有進沒有出的桶,最後會裝著一堆已經修好的東西**,
#   然後它的計數就不再有人相信。
#
#   ### ★ **而本工具自己就是那個門檻的載體 —— 它剛剛親自來要求移。**
#   cp 完成的那一次執行,它印的是:
#     「~ 寫法(清單自己)  擋下了 —— 缺口可能已被補上,請回頭收 票 88」
#     「不合格:['~ 寫法(清單自己)(已知缺口變成擋下,票面未更新)']」
#
#   **而「不合格」這個措辭是對的**:它不說通過,因為
#   **票面與現實對不上,而對不上就是不合格,不管方向是哪一邊。**
#   一個只在「變壞」時才出聲的驗收工具,會讓「變好」變成靜默的漂移。
#
# **樣本一律用無害動詞。** 第一級**不分讀寫、判準是路徑**(它的擋下訊息自己
# 這樣寫,並經 2026-08-28 實測:同一個受保護檔,`cat` 也照擋)——
# 所以讀取就足以示範一個缺口,不需要破壞性指令。
# 而這一格**隨 `copy` 進每一個下游 repo**:不該在別人的樹裡放一條 `rm -rf` 樣本。
# (既有那條 UNC 樣本保留原樣 —— 舊文不改,見票 80;新增的照本規矩走。)
KNOWN_GAPS = [
    ("bash 寫法 UNC", r'rm -rf //server/share/x', "票 80(裁 A 明訂不動)"),
    # ── 2026-08-31 移出:`~ 寫法(清單自己)` → 票 88 ───────────────────────
    # 舊條目逐字(照 `F-036` 留痕,不刪):
    #     ("~ 寫法(清單自己)", r'cat ~/.claude/g1-protected.txt', "票 88")
    # 它記的是:第一級不展開 `~`,同一個檔案 `C:\Users\…\.claude\g1-protected.txt`
    # 擋下、而 `~/.claude/g1-protected.txt` 放行 —— **這一條的受害者是守衛自己**。
    #
    # **出的門檻(見檔頭)已滿足**:票 88 那一刀補了 `variants()` 的 `~` 形態,
    # Jeff 於 2026-08-31 完成人工覆蓋(備份 `.working6`,三格全對),
    # 本工具當場自己來要求收票:
    #     「~ 寫法(清單自己)  **擋下了** —— 缺口可能已被補上,請回頭收 票 88」
    #     「不合格:['~ 寫法(清單自己)(已知缺口變成擋下,票面未更新)']」
    # live 探針同日複驗:`ls ~/.claude/g1-protected.txt` → `[G1/保護清單]` 擋下,
    # 訊息指名完整路徑。**⇒ 移出。**
]
