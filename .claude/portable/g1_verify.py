...
KNOWN_GAPS = [
    ...
    # 第一級不展開 `~`:同一個檔案,`C:\Users\…\.claude\g1-protected.txt` 擋下,
    # `~/.claude/g1-protected.txt` 放行。清單條目由 `variants()` 展開成
    # 反斜線 / 正斜線 / 大小寫 / 磁碟代號 / MSYS / WSL 六種形態,**`~` 不在表裡**
    # (`expanduser` 全檔只用在定位守衛自己的家目錄那一行)。
    #
    # **這一條的受害者是守衛自己** —— `ADR 0009` 的遞迴(「agent 改不動它」)
    # 靠的就是清單裡有這一行,而某些寫法讓那一行等於不存在。
    # 樣本用 `cat`(無害讀取),理由見上。
