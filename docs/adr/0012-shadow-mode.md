...
>
> **已裁(2026-08-26):標成從未實作,不刪、不補** —— 與 `窗 14 天` 同樣處置。
> 三條理由:
> **(a)** 三個門檻值沒有出處(`F-124`),**把沒有理由的數字變成閘門,
> 等於把一個猜測升格成規則**;
> **(b)** 補實作 per-rule 晉升是一大塊新工作,**12 天窗口內不該開**;
> **(c)** 全局 enforce 已有活體對照組(上游 82 張票全在 enforce 下累積)——
> 但那是**有理由樂觀,不是已經驗證**,兩邊碼況不同(上游從零就在閘門下長,
> 量化有既存違規,`legacy-no-redlight.txt` 126 行正是為此而設)。
>
> **三條門檻保留為報表**,而 `shadow_review --status` 的輸出與
> `promotion_status` 的 docstring 現在會**自己說出「報表,非閘門」**
> (釘子:`TestFTheThresholdsAnnounceThatTheyAreNotAGate`)。
> 加測試而不只加註解,是因為**報表與閘門在文件裡長得一模一樣** ——
> 兩者都用「條件…才可…」的句式,而「可轉正」三個字讀起來就是一道門的判決。
