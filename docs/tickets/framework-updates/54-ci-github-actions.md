...
| **淨室(安裝後形態)** | ~~**這台機器上直接跑會中斷** —— 主控台是 cp950,`verify_gates.py:211` 印 `✓` 時丟 `UnicodeEncodeError`;**要 `python -X utf8` 才跑得完**~~ → **`-X utf8` 已不需要(票 62,2026-09-07)**:輸出改走 utf-8 位元組,不帶旗標直接跑 **`exit=0`**、9 條規則各擋一次、巢狀 `1262 passed, 3 skipped, 3 xfailed`。**本機對「規則擋不擋得住」的涵蓋從 0 條變成 9 條** | **有**(每次 CI,Linux 主控台是 UTF-8) | **票 56 加入;本機那一格由票 58 D4 更正、2026-08-19 再更正位置(F-107)、2026-09-07 由票 62 消掉。** CI 綠燈涵蓋 **13 / 93 面**的真實形態,**不是全部** —— 見下方註 |
| **`.githooks/` 的 hook 在 Linux 上真的被 git 執行** | **0** —— Windows 的 git 不看執行位元,測不出來 | **0** —— **CI 從不 commit** | **兩邊都是零。** 票 58 加入,見下方專節 |
| 🔴 **本機不同殼(PowerShell vs Git Bash)** | ~~**PowerShell:`4 passed, 5 skipped`**;Git Bash:全跑~~ → **票 96 落地後兩邊都是 `16 passed`** | **有**(ubuntu 上 `sh` 一定在,那 5 條在 CI 一直是真的跑的) | **票 96 加入(2026-09-07)。這一列本來不存在** —— 原表記的是**CI 相對本機**的落差,**沒有記「本機在不同殼下的落差」**,於是一個「本機綠燈的範圍比它看起來小」的狀態沒有任何一格裝得下它。**差異消失的條件見下方註** |
| R1–R5、R7、R8、G1 | — | — | 無差 |

### 🔴 「本機不同殼」那一列 —— 差異消失的條件(票 96,2026-09-07)

**差異在下列條件同時成立時消失,缺一就回來:**

1. `tests/test_bootstrap.py` 的 `find_sh()` 依序試 **`sh` → `bash` → Git for Windows 標準路徑**
2. 走寫死路徑那條時,`sh_env()` 把 **Git 的 `usr\bin` 前置到子行程的 PATH**
   —— 少了這一步那 5 條會**跑起來然後紅在 `cut: command not found`**,
   而三道檢查一道都還沒跑到
3. Git for Windows 裝在**預設位置**

**⇒ 條件 3 不成立時仍然 skip,而 skip 訊息會照實說**(它本來就在說實話,票 96 明訂不得弱化)。

> ### **這一列教的東西比它記的數字重要:**
> **一張「A 相對 B」的落差表,量不到「B 自己內部的落差」。**
> 原表的兩個欄位是「本機」與「CI」,而**「本機」在這台機器上有兩個值** ——
> 於是那個落差**沒有一格裝得下它**,而不是被記成零。
> **一份對照表的盲區,長在它的欄位定義裡,不長在它的內容裡。**

### ⚠ 這張表有**兩個方向**的盲區,而它們不會互相補上(2026-08-19)

讀這張表最容易犯的錯,是把它讀成「CI 是本機的超集」——
**它不是。兩個方向各有對方看不到的東西。**

| 方向 | 只有哪一邊看得到 | 標本 |
|---|---|---|
| **本機看不到,只有 CI 看得到** | symlink 三條(Windows 建不了)、`.githooks` 在 Linux 上真的被執行、~~**淨室**(cp950 中斷,要 `-X utf8`)~~ **淨室那一項 2026-09-07 由票 62 移除** —— 本機現在看得到了 | 票 62 / 上面那張表的「淨室(安裝後形態)」列 |
| **CI 看不到,只有本機看得到** | **個人 pattern 那一層** —— `~/.claude/leak-patterns.local.txt` 不進版控、不上 CI | 2026-08-19:票 68 的測試 fixture 寫了一個逼真的假路徑,**命中個人 pattern #17 而在本機轉紅**;那一版 **在 CI 上會全綠** |

> **一個「本機綠 + CI 綠」的組合,涵蓋的仍然不是聯集** ——
> 因為兩邊各自跳過的東西不同,而**跳過的部分不會出現在任何一邊的輸出裡**。
>
> **具體的危險形狀**:有人為了讓本機綠而把一個測試改成不觸發個人 pattern
> (那是對的),或為了讓 CI 綠而把一個 Windows 跑不動的東西 skip 掉(那也是對的)
> —— **兩個都對的動作,合起來會讓一塊區域對兩邊都隱形。**
>
> 判準:**問「這一項在哪一邊被跳過」,而不是「兩邊都綠了嗎」。**

(這一段原本要放進票 62,裁決 2026-08-19 判落在這裡:
票 62 講的是 **cp950 那個缺陷**,而本段講的是**這張涵蓋圖的結構** ——
它對票 62 修好之後仍然成立。票 62 留一條指標指過來。)
...
#### ⚠ 2026-08-18 CI 實測之後,這一列仍然是 0/0

那一次 CI **綠**:`sh bootstrap.sh` 在 Linux 全新 clone 上通過三道並設下
`core.hooksPath -> .githooks`,外層 814 passed。**而這一列一格都沒動。**

那一次證明的是「**index mode 隨 clone 走**」——
檢查③ 讀的是 `git ls-files -s`(**index**),而 CI **仍然不 commit**。
所以它**沒有**證明「checkout 出來的檔案可執行」,也**沒有**證明「git 會執行它」。

> **一次綠燈通過了它旁邊的路,沒有走進這一格。**

**這一列因此從「缺乏證據」升級成「不在場的證據」** ——
推之前那是推論(我們沒驗過);推之後那是**一次真實的、完整的、綠的 CI 執行
仍然沒有觸及它**的紀錄。**後者比前者硬。**

> **綠燈在這一格不是證詞,是沉默。**
> **而沉默與「驗過了沒事」在紅綠燈上長得一樣** —— 那正是這一列存在的全部理由。

**推之前寫下的預先診斷**(「若紅,最可能是 checkout 之後的實際 mode 與 index
不一致」)**既沒被驗證也沒被推翻** —— 因為**沒有任何東西去看過 checkout
出來的那個位元**。留著不刪:那個假說仍然待驗,而補法(`verify_gates` 加一個
跑 bootstrap 再 commit 的情境)就是驗它的方式。

> **這一列自己就是 `F-099` 第五列的載體(落差表)。**
> 而它現在正在**履行自己的功能**:攔下「CI 綠了 ⇒ 權威層應該沒問題了」這個誤讀。
> **一張落差表最該證明自己的時刻,就是它旁邊剛亮了一盞綠燈的時候。**
...
> **⚠ 上面那一段的位置寫反了,2026-08-19 實測更正(照 F-036 舊文不刪)。**
>
> 斷點在 R1–R8 逐條實測**之前**,不是之後。程式碼順序(`verify_gates.py`,
> 且**與 `69854e9` 當時逐行相同 —— 順序沒變過,是描述錯的**):
>
> | 行 | 段 |
> |---|---|
> | 194 | `=== 真實安裝(不是簡化版)===` |
> | **210-212** | **`=== 安裝器預設值(F-062)===`  ← 炸在這裡** |
> | 216 | `=== 規則清單 ===` |
> | **225** | **`=== 逐條實測(每條各擋一次)===`  ← 從來沒跑到** |
> | 242 | `=== 權威層偵測 ===` |
> | 266 | `=== 框架自己的測試,在這個新 repo 裡跑一次 ===` |
>
> 2026-08-19 實測輸出的末段(崩潰前最後印出來的東西)是
> 「去識別化/影子模式」與 R2 探針那一段,**沒有任何一行 `R1  擋下 ✓`**。
>
> **更正之後這一列變得更嚴重,不是更輕:**
>
> | | 舊描述 | 實況 |
...
> | 巢狀測試數 | (未提) | **本機從來沒有量到過** |
> | 讀者會得到的印象 | 跑到一半才壞 | **這台機器上,淨室對「規則擋不擋得住」的涵蓋是零** |
>
> **舊描述比現實樂觀,而這一節的標題就是〈落差表比現實樂觀〉。**
> 一則專為修正「比現實樂觀」而寫的更正,自己的細節也比現實樂觀 ——
> 已落成 `F-107`。
>
> **`python -X utf8` 那個出口沒有被推翻**:加了它就跑得完(該出口寫在本表第 24 行)。
>
> ### 再更正(2026-08-19 稍晚):上面那句「本機從來沒有量到過」**已經不成立**
>
> 同日為了診斷 `152cb62` 的 CI 紅(**後來查明是 GitHub 平台故障,非我方**,見 `F-111`),
> 我第一次真的用 `-X utf8` 把淨室跑完 —— **這台機器上的第一次**:
>
> ```
...
>     891 passed, 3 skipped, 3 xfailed in 67.30s        (淨室內的框架測試)
> EXIT=0
> ```
>
> 所以那張表的三格現在是:
>
> | | 更正前寫的 | 2026-08-19 稍晚的實況 |
> |---|---|---|
> | 本機看得到什麼 | R1–R8 一條都沒看過 | **八條全部看過,全部擋下** |
> | 巢狀測試數 | 本機從來沒有量到過 | **量到了:891**(Linux 再 +3 條 symlink = 894) |
> | 淨室複製檔數 | (未提) | **91** |
>
> **但「本機直接跑會中斷」那一句仍然成立** —— 出口要人**記得加 `-X utf8`**,
> 而它**不在任何可執行的位置**(不在 `bootstrap.sh`、不在 CI、不在工具自己的 usage)。
> 票 62 的判準沒有被推翻:**一個只寫在落差表裡的解法,救得了讀過落差表的人,救不了執行的人。**
>
> 三次更正同一列(票 56 加入 → 票 58 D4 → 2026-08-19 位置 → 2026-08-19 稍晚實測),
> 每一次都往「更接近實況」移動一格。**舊文全部留著** —— 那條軌跡本身是這一列的價值。
> 錯的是「不加它會跑到哪裡」。
...
- 不裝 `gh`
  (註記 2026-08-25:此行為當時的範圍劃定,無附理由;已由本日裁決取代,
  非永久禁令 —— 准裝,授權範圍見 friction-log `F-119` 補記)
