...
| **淨室(安裝後形態)** | **這台機器上直接跑會中斷** —— 主控台是 cp950,`verify_gates.py:211` 印 `✓` 時丟 `UnicodeEncodeError`;**要 `python -X utf8` 才跑得完**。**斷點在 R1–R8 逐條實測之前(2026-08-19 更正),所以本機對「規則擋不擋得住」的涵蓋是 0 條、巢狀測試數從未量到** | **有**(每次 CI,Linux 主控台是 UTF-8) | **票 56 加入;本機那一格由票 58 D4 更正、2026-08-19 再更正位置(F-107)。** CI 綠燈涵蓋 **13 / 93 面**的真實形態,**不是全部** —— 見下方註 |
...
| 方向 | 只有哪一邊看得到 | 標本 |
|---|---|---|
| **本機看不到,只有 CI 看得到** | symlink 三條(Windows 建不了)、`.githooks` 在 Linux 上真的被執行、**淨室**(cp950 中斷,要 `-X utf8`) | 票 62 / 本表第 24 行那一列 |
