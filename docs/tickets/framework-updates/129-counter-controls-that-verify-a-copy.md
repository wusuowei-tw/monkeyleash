...
### 動工順序(2026-09-09 裁決追加,上面的文字不改)

**本票動工第一步 = 突變**:把 `tests/` 那支 `_imported_names()` 打樁成回空集合,
跑全套,量它是不是仍然全綠。
**先把「它擋不住」從推論變成量測,再盤點同族。**

**理由(裁決者 2026-09-09 原話)**:目前 129 的主張全部來自讀碼,
而這個突變一次就能把它變成量測。

> **⚠ 這與上面〈真正的題目〉開頭那句「第一步是全庫盤點」是【順序的細化】,不是推翻。**
> 舊句管的是「不要修完兩條就收工」,新句管的是「盤點之前先把主張坐實」。
> 合起來的順序是:**突變 → 盤點 → 才談修**。
> 舊句不刪(`F-036`)。
>
> **為什麼突變排在盤點前面**:盤點會產出一份清單,而**清單看起來就像處置**
> (`CLAUDE.md`「識別風險的品質越高,它偽裝成處置的能力越強」)——
> 在主張還是推論的時候先產清單,得到的是一份**沒有人驗過前提**的清單。
> 而突變便宜:一次打樁、一次全套。

**這個突變本輪【沒有跑】** —— 裁決逐字禁止(「不要跑那個突變」)。
它仍在下面的〈未證明〉裡,**那一格要等真的跑過才劃掉。**
