...
**CI 綠,外層 814 命中兩份獨立預測,驗收 10/10。**
**⚠ 勾滿不等於這件事的證據齊了** —— 票 54 落差表新增的那一列仍是 **0/0**,見文末。
...
- [x] **CI 的「接上權威層」步驟綠** —— 2026-08-18 實測:三道 fail-closed 在
      Linux 全新 clone 上第一次真的跑過且全過,輸出 `core.hooksPath -> .githooks`。
      **⚠ 它證明的是三道「接受」了條件,不是「hook 會被執行」** —— 見票 54 落差表
      那一列(**仍是 0/0**)
...
## CI 實測與三路對帳(2026-08-18)

| | 預測 | 實測 |
|---|---|---|
| 本機 | — | **824 passed / 3 skipped / 3 xfailed** |
| **外層 CI** | **814 passed / 1 deselected / 3 xfailed / 0 skipped** | **814 —— 命中** |
| 巢狀(淨室) | 793 | **未取得** |

**巢狀那一格不填預測值。** 沒拿到就是沒拿到 ——
填一個「應該是 793」進紀錄,就是把預測寫成實測,**而那兩者在表上長得一樣**。

外層兩份預測由**兩條不同路徑**導出(裁決者走「本機 passed 加減」、
本輪走「collected 扣除」),落在同一個數字上。

### 這一次的綠證明了什麼(兩件,都不是我原本算到的)

**一、接受路徑與拒絕路徑是兩個不同機制,分別在 Linux 上驗的。**

| 路徑 | 由誰驗 |
|---|---|
| **接受**(條件滿足 → 設下 config) | workflow 的 `sh bootstrap.sh` 步驟 |
| **拒絕**(三種違規 → 擋下且不設 config) | `tests/test_bootstrap.py` 的 **3 條行為正對照**,在 814 裡面,**Linux 有 `sh` → 零 skip** |

**所以不是只驗了「它沒擋我」。** 兩半各自有機制,而且都在 Linux 上跑過。

**二、這一次的綠不是空的 —— 它有一個確定會紅的前身。**

`e195adc` 之前 `.githooks/pre-commit` 的 index mode 是 `100644`;
**三道若當時就在,CI 會當場紅。**

> **判準(裁決者立,2026-08-18):一個從來不會紅的綠燈是空的。**
> 判一個綠燈有沒有內容,問的是「**有沒有一個確定會讓它紅的狀態**」——
> 而那個狀態最好是**真的存在過**,不是想像出來的。
> 本件的那個狀態有日期、有 blob、有 commit:`100644`,`e195adc` 之前。

**這條判準與票 47(R5 零正對照)是同一個問題的兩面** ——
票 47 問「這條規則有沒有被證明會動」,本條問「這個綠燈有沒有可能紅」。
**兩者的反面都是同一句:證明規則存在,不證明規則會動。**
