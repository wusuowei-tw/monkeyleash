...
## 零之一、處置:**兩件一起做,排在【上線之後】**(裁決 2026-08-28)

**裁准**,但**排程改過兩次,兩次的理由不同,兩次都記下來:**

| 何時裁 | 排在 | 理由 |
|---|---|---|
| 2026-08-28(早) | ~~8/31~~ | 8/28 剩餘時間歸第 1 級(搬家不可逆線) |
| **2026-08-28(晚)** | **上線之後** | **上線日提前到 8/31,而上線當天不動 CI** |

### 🔴 為什麼上線當天不動 CI

**三保護是綁 status check 名稱的,而 8/31 是「當天改、當天設」** ——
名稱一旦跑掉,**沒有時間補**。

> **改 CI 與設保護放在同一天,等於讓兩件事互相當對方的前置,
> 而它們之中先做的那一件出錯時,後做的那一件才會發現。**
> 上線當天沒有第二次機會:保護沒綁上就翻公開,窗口是開著的。

所以本票的改動**排在上線之後**,那時 `tests` 這個名稱已經被保護規則綁定,
改動可以先在一次普通推送上驗證名稱有沒有跑掉,再決定要不要保留。

**本票時鐘因此與上線日脫鉤** —— 原本寫「9/1 之前」是因為它被當成上線前置,
而它不是:兩邊清單對不對得起來,**與 repo 公不公開無關**。

### (a) 加 `--collect-only` 步驟 —— **用途是證明「兩邊收集的是同一批」**

**這一句要寫在票面,因為用途決定了它做完之後怎麼讀。**

它**不是**用來找那 3 支 skip 的(那 3 支兩邊都收集得到,見零之二 ——
`--collect-only` 對它們的差集是空的)。它答的是**第 1、2 類差集**:

- 只在本機收集到的
- 只在 CI 收集到的

> **把用途寫錯的後果很具體**:做完之後看到差集是空的,
> 會被讀成「那 3 支不見了的問題已經解決」——
> 而實際上那個問題**根本不在這把尺的量程裡**。
> 一個答對了自己問題的工具,被拿去回答另一個問題時,**它的綠燈是誤導的**。

### (b) 現有 `pytest` 步驟加 `-rs` —— **答第 3 類**

`-rs` 讓 short summary 印出**每一支 skip 的節點與理由**,而現在 CI 那一步
只印得出「有幾個」(而且這一輪印出來是 **0 個**,所以連「有幾個」都看不到形狀)。

**零額外執行成本** —— 它只改報告的詳細度,不多跑任何測試。

> **這把尺是 2026-08-28 照出「第 3 類沒有尺」之後才補得上的。**
> 第一節的驗收從立案起就列了三類差集,而第 3 類
...
**條件一:job / workflow 名稱不得改動。**
**8/31 上線當天要設的** Require status checks **是照名稱綁的** ——
名稱一變,那條保護規則會靜默地不再對應任何檢查
(規則還在、而它守的東西不見了)。
**改完要實測確認 `tests` 這個名稱仍出現在 CI 結果裡**,不能只看 diff。

**條件二:新步驟只收集不執行,不得影響現有 `pytest` 步驟的旗標或結論。**
新步驟自己紅不紅是另一回事;**它不得讓原本會綠的那一步變紅或變綠**。

**條件二對 (b) 的讀法**:`-rs` 是加在**現有那一步**上的,所以它天然踩在
這條界線上。判準寫死:**`-rs` 只改報告的詳細度** ——
它不得改變 `addopts`、不得改變選取集合、不得改變退出碼。
改完拿本輪的基準對照:`1054 passed, 1 deselected, 3 xfailed`
**這三個數字必須逐字不變**,變了就是踩線。
...
## 🔴 2026-08-28:一次**對得上的算術**,而本票**不收**(裁決)

票 89 那一輪的本機與 CI 數字**完全對得起來**:

```
本機收集 = 1108 passed + 3 skipped + 3 xfailed        = 1114
CI  收集 = 1098 passed + 1 deselected + 3 xfailed     = 1102
差 12    = --ignore=tests/test_known_items_regression.py 那一檔的 12 條
CI 內部  : 1102 - 1 deselected = 1101 跑;- 3 xfailed = 1098 passed  ✓
本機那 3 個 symlink skip 在 Linux 上會過,已含在上式裡
```
...
> ### **本票的驗收語言要求的是【集合差集】,而上面那串是【算術】。**

本票第 63-64 行寫死了驗收形狀:「只在本機收集到的」/「只在 CI 收集到的」——
**兩份逐條清單相減**,而 `--collect-only` 那一步**還沒做**。

上面那 12 的來歷是**我從 `tests.yml` 的 `--ignore` 讀出來、再回頭對上數字的**,
不是從兩份清單相減得到的。**我沒有驗過那 12 條就是那一檔的那 12 條。**

### 🔴 而 2026-08-28 這一天已經三次證明「數字對得上 ≠ 集合相同」

| # | 場景 | 數字看起來 | 集合實際上 |
|---|---|---|---|
| 1 | 兩台的個人 pattern 清單(票 91) | 筆電 **19** / 桌機 **18** —— 像是「多一條」 | **差集 5 條**:筆電獨有 3、桌機獨有 2 |
| 2 | stale status 的舊告警分桶 | 「告警 8 筆」 | 誤報 2 + 可爭議 3 + 真陽 2 = **7**,少一筆沒有桶(`F-113`) |
| 3 | 本節這串 | 差 12,**完全對得上** | **沒有人比對過那 12 條的身分** |

**第 1 個最有力**:`19 - 18 = 1` 這個算術**完全正確**,
而照它去補會**補錯 3 條又漏掉 2 條**,然後**兩邊條數看起來會變成一樣**。

> ### **一個對得上的算術,會讓「已經查過了」的感覺比實際強一階 ——
> 而它正是本票存在的理由。**

### ⇒ 裁決:**不得用一個剛好對得上的算術,收掉一張要求集合證據的票**
