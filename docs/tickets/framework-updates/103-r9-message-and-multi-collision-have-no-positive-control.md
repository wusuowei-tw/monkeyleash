...
## CI(run `33847385546`,`success`,`pytest in 51s`)

四刀(`68ba246` / `43b08f7` / `b98450b` / `180d059`)於 2026-09-04 推上
`ba578e5..180d059`,**未用 force**。CI 原文:

```
跑測試                             1260 passed, 1 deselected, 3 xfailed in 17.17s
淨室驗證(每條規則各擋一次 + 安裝後形態)  1178 passed, 3 xfailed in 10.89s
...
|---|---|---|
| 本機 passed | **1270** | 收刀那一跑 |
| − 個人 pattern 那一檔 | **−12** | `tests.yml:71` 的 `--ignore=tests/test_known_items_regression.py`;**12 是本輪重量的**:`python -m pytest tests/test_known_items_regression.py --collect-only -q` → `12 tests collected in 0.02s` |
| ＋ Windows 才 skip 的 symlink 三條 | **+3** | 本機 `3 skipped`(`test_gate.py:451/459/473`,理由「此環境無法建立 symlink」);ubuntu 上它們會跑而且過,所以進了 CI 的 passed |
...
| **= CI passed** | **1260** | **與實測相符** |

`xfailed` 兩邊都是 **3**,一條不差。`deselected` 預測 1、實測 1。

> **`1270 − 12 + 3 − 1 = 1260` 是四個各自有指名來源的數字相加,不是先看到 1260 再湊出來的。**
> **這個 1260 是在拿到 CI 實測【之前】寫下的**(見刀四回報),
> 而票 102 §CI 逐字警告過:「閉合是算術,不是歸因」。
> **−12 那一列本輪重新量過**,沒有從票 102 抄 —— 票 102 自己對票 85 做過同一個動作。

### 淨室那 1178 —— **不與上表對帳,但它自己有一個基準**

照票 102 的處置:**淨室跑的是另一棵樹**(安裝後的新 repo,只有框架自己的測試),
與「本機 passed」**沒有可比的基準**,所以**不併進上表**。
**明寫「沒對」,不是「對得上」** —— 這一格留白比蓋章誠實。

**但它與【票 102 的淨室】可比**(同一種樹、同一支指令):

| | 淨室 passed |
|---|---|
| 票 102 收刀(run `33751941469`) | **1169** |
| 本票(run `33847385546`) | **1178** |
| 差 | **+9** |

**`1169 + 9 = 1178`,而 9 正是本票新增的條數** ——
中間沒有任何其他 `.py` 進出(三筆純文件 commit + 一筆票面)。

> ⚠ **這一格是本票【新加】的觀測,不是票 102 建立的慣例。**
> 它問的是另一個問題:**「淨室這棵樹有沒有跟上新測試」** ——
> 而那件事上表答不了(上表的基準是本機那棵樹)。
> **兩個問題、兩個基準,所以不是同一個檢查跑兩遍**(`F-108`)。
