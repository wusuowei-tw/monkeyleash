...
## 2026-09-24 併入:票 137 的盤點結果(第 3 條〈未證明〉的部分進度)

出處:`.dev/reports/2026-09-24T063336Z-ticket137-recon-standalone.md` §6.1–6.3。
票 137(立案 2026-09-12)**與本票是同一個缺陷**,立案時沒有引用本票;
比對確認**是同一台機器** —— 本票引的第一筆紀錄時間
`2026-09-04T00:29:05.079605+00:00`,與票 137 那台的帳本逐字相同
(本票當時記 8 筆,2026-09-24 為 38 筆,同一份檔案繼續長)。

### 量到的(單位:筆 / 支)

| 量測 | 值 |
|---|---|
| go-live 樹(`91335c8e…`)全部路徑 | 91 支 |
| 其中 `.py` | 20 支 |
| 其中 `is_source_path` 為真(= 生成集合 `expected`) | 10 支 |
| `.agents/legacy-no-redlight.txt` 現有條目(`entries`) | 9 筆 |
| `entries − expected`(方向一,只減不增) | **0 筆** |
| **`expected − entries`** | **1 筆:`.claude/portable/g1_guard.py`** |
| `expected` 之外的 10 支 `.py`(生成器與測試都看不到) | 10 支,**全在 `tests/`** |
| HEAD 現樹的原始碼 `.py` | 19 支;其中 9 支不在 go-live 樹裡(新檔,永遠進不了清單,靠真紅燈過 R3) |

### ⚠ 這次盤點**證明了什麼、沒證明什麼**

- **證明了**:**本次 `expected − entries` 僅有 `.claude/portable/g1_guard.py` 一筆** ——
  那條紅**沒有漏報同類**(方向二掃的是差集全部,不是抽樣);
  而生成器與測試共用的 `is_source_path` 盲區裡,10 支全是 `tests/` 底下的檔案,
  **沒有「裝著判定邏輯卻被判非原始碼」的 .py 混在裡面**。
- **沒有證明**:**「全庫只有一條測試依賴本機帳本」。**
  上面量的是清單的差集,不是「哪些測試會讀 `.dev/test-runs.jsonl`」。
  **本票〈未證明〉第 3 條因此仍然開著。**

### 本票的正題沒有被票 137 解掉

票 137 的裁決(C 最小版)是**讓那條完整性測試不再依賴帳本**,
做法是把排水紀錄改成機器可讀、寫在清單檔裡跟著版控走。

**那是繞過,不是回答。** 本票問的仍然是:
`.dev/test-runs.jsonl` 是**本機執行紀錄**,還是**專案的紅綠燈憑證**?
**R3 的紅燈半判定本身仍然踩在這份 per-machine 證據上**,一個字沒動。
