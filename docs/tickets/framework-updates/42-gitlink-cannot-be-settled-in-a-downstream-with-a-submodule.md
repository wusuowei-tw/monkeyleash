...
### 補做(裁決 A 的條件,2026-08-15 第二輪)

裁決:**維持兩份 + 一致性測試**。條件是那條一致性測試**自己要被突變量過**,
否則它是裝飾。

做法不是跑一次一次性突變,而是**把突變做成常駐測試**:
`test_the_parity_check_itself_goes_red_when_one_side_drifts` ——
只改壞 scanner 那一份的 `GITLINK_MODE`(改常數不改檔案,那份是測試自己載的),
斷言一致性檢查必須紅。斷言抽成 `_assert_parity()` 讓兩條測試打到**同一段**程式碼 ——
複製一份斷言來測「斷言會不會紅」,測到的是那份副本(ADR 0003 的形狀)。

**寫的時候自己踩了一次 F-090**:第一版把 `self._repo(tmp_path)` 放在
`pytest.raises` 區塊**裡面**,而 fixture 自己有一句 `assert mode == "160000"` ——
fixture 一壞,這條測試會**照樣綠**,綠的理由變成「fixture 炸了」。
已改成在區塊外建 repo,並額外斷言**紅的理由是那句一致性斷言**。
記在這裡是因為:**這條測試存在的理由,就是我在寫它的時候犯的那個錯。**

同時發現 **`F-058` 在本 repo 是撞用的**:本檔的 F-058 是「資料內容誤觸 R7」,
而 `gate.py` / `scanner.py` / `sync.py` / `manifest.py` 與多張票引用的 F-058 是
「一個事實兩個來源」—— 後者的原始條目住在框架抽出前的宿主 repo。
**未動任何編號**(引用是在當時的號碼下寫的,改號會讓引用斷掉),
處置是在 friction-log 的 F-058 底下把撞用寫出來。合流仍是待裁決項。

**沒做的一件,誠實寫出來**:`scanner.py` 那一側**沒有**加上「刻意保留兩份」的註解
(`gate.py` 那側本來就有)。理由是 R3 —— 上一個 commit 之後 HEAD 動了,
票 42 的紅燈紀錄 `impl_hash` 已經對不上現在的 HEAD,再寫 `scanner.py` 會被擋;
而為了加一行註解去湊一次紅燈,正是規則要防的自我服務。
註記已經寫進 friction-log 的 F-058 家族註記(兩份實作、理由、由哪條測試看著),
那是查得到的位置。要補進 `scanner.py` 的話,跟著下一次動那支檔案的票一起做。
