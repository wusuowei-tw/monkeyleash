...
### ⭐ 為什麼這支腳本一定要由 Jeff 跑 —— **活例子,2026-09-23**

第一版腳本在 agent 環境裡通過了所有靜態核對。
**裁決者在普通 PowerShell 首跑,第 1 案就停:**

```
前置/誤用:找不到 sh/bash —— 跑不了 bootstrap.sh
```

**病因不是 `bootstrap.sh`,也不是那台機器缺東西** ——
Git for Windows 就裝在預設位置,`C:\Program Files\Git\usr\bin\sh.exe` 在。
病因是**腳本自己寫了 `shutil.which("sh")`**,
而 **agent 環境的 PATH 上剛好有 `sh`、普通 PowerShell 沒有**。

> ### **那是環境等價性缺陷:腳本只在一種殼裡被驗過,而它要跑的是另一種。**
> **靜態核對、`py_compile`、單元測試全綠,一個都抓不到它** ——
> 因為它們全都跑在同一個有 `sh` 的環境裡。

**這正是「REAL 層材料要由人在真實環境跑」擋得住、而 agent 自起擋不住的那一類。**
不是因為人比較聰明,是因為**人的終端機是另一個環境** ——
而「另一個環境」正是這一層要驗的東西。

⚠ **同時也要看清楚它證明的範圍**:它暴露的是**腳本的**環境假設,
**不是**權威層本身的任何事情。第 1 案就停 ⇒ 六格**一格都還沒驗到**。
「人跑的」解決的是材料來源,**不解決設計對不對**(見 §九)。

**修法(2026-09-23)**:與**票 96 同形**,而票 96 已經裁過解法(方向 3)。
所以**不寫第三份找殼邏輯**,直接重用 `tests/test_bootstrap.py` 的
`find_sh()` / `sh_env()`,並**順手關掉票 96 自己標的那個已知缺口**:

| 層 | 找法 | 來源 |
|---|---|---|
| 1 | PATH 上的 `sh` → `bash` | 票 96 原有。**PATH 優先** —— 使用者殼裡有什麼就用什麼 |
| 2 | 寫死的 `C:\Program Files\Git\usr\bin\sh.exe` / `bin\bash.exe` | 票 96 原有 |
| **3** | **`git --exec-path` 往上三層推出 Git 安裝根**,再試 `usr\bin\sh.exe` / `bin\sh.exe` / `bin\bash.exe` | **新增** —— 關掉票 96 逐字標的「裝在非預設磁碟/目錄」缺口 |

實測(裁決者本機,與他自己推的一致):

```
git --exec-path   -> C:/Program Files/Git/mingw64/libexec/git-core
往上三層          -> C:/Program Files/Git
usr\bin\sh.exe    -> exists=True
```

**順序刻意是 PATH → 寫死 → 推導**:推導要跑一個子行程,排在後面表示
常見情形不必付那個錢;而且**不會改掉票 96 既有測試問的問題**(①②仍然綠)。

**`sh_env()` 一起帶過來**,那是票 96 第二個缺口換來的:
叫得動 `sh.exe` 卻沒有 `cut` 的環境**不是**跑得動 `bootstrap.sh` 的殼,
會紅在 `cut: command not found` —— **而那種紅看起來像 `bootstrap.sh` 壞了**。
推導來的殼前置的是**它自己那個安裝根**的 `usr\bin`,不是寫死的那一個。

**找得到**:印出實際用的那一支,**只印相對於 Git 安裝根的位置**
(`<Git 安裝根>/usr/bin/sh.exe`),不印完整路徑。
**都找不到**:照舊停在「未完成」,而訊息**列出試過的每一個位置**與推出的安裝根。

⚠ **已知耦合,原地登記**:`find_sh` / `sh_env` 住在一個**測試模組**裡,
而腳本是維運程式。要它們搬到共用位置是另一個決定(會動到測試佈局),
**本輪不做** —— 本輪的判準是「不要再寫第三份」。
這個 import 壞掉會**當場 ImportError**,不會靜默退化。
...
## 三之四、🔴 構造前提要被斷言 —— 兩格意外提交成功換來的

煙霧測試(2026-09-24)在 `8-reject` / `9-reject` 兩格撞到**意外提交成功**。
**唯讀核對保留的 clone 之後,確認【不是閘門失效】**:

| 核對 | 結果 |
|---|---|
| 兩筆意外 commit 的 `--stat` | 各只有 1 個檔(`redlight.py` / `research/probe.py`),**沒有定義檔** |
| commit 裡的 `.agents/pipeline-stages.yaml` blob | `33a5c749…` —— **與基準逐字相同** |
| `staged` 清單 | 沒有 `.agents/pipeline-stages.yaml`(**且清單沒有濾掉 `.yaml`**,缺席是真的) |

**根因**:構造做了 `git add -- <定義檔>`(`check=True`,成功),
下一行卻是 `git checkout HEAD -- <定義檔>` ——
而 **`git checkout <tree-ish> -- <path>` 會同時重設 index 與工作樹**,
把前一行的 `git add` 抹掉了。

> ### **違規構造於是靜默退化成合法構造,而結果與合法對照【無法區分】。**
> 兩格都「跑完了」、都印出 commit、都有 `trace2 hook=True` ——
> **沒有任何東西說前提已經不成立了。**

**修法兩半:**

1. **只還原工作樹**:用 `git show HEAD:<path>` 取內容再自己寫檔 ——
   **那條路只讀,不可能碰到 index**。
   所有必要的 git 操作一律 `check=True`,**不以 `check=False` 吞掉失敗**
   (原本那一行正是 `check=False`)。
2. **每一格都補【構造前提斷言】**,在 commit **之前**跑;不成立 ⇒ 記
   **「未完成」且不執行 commit**。
...
| 格 | 斷言的是 |
|---|---|
| `8-reject` | 定義檔**在** index;**解析兩份 YAML**:index 的 `implement` 確實落進「前置站」集合且不再是可寫站、**工作樹仍是基準**;且 index 仍有可寫站(否則會變 `[R2/fail-closed]`,是另一格) |
| `8-reverse` | 定義檔**不在** index;方向相反,且兩份內容**確實不同** |
| `9-reject` | index 的 `research` 確實**移除** `exempts_r3_in_scope`、工作樹**仍保留**;且 index 的 `research` 仍可寫且 `src_write_scope` 還在(否則落到 `[R2/範圍]`) |
| `10-reject` | 假條目**在工作樹清單、不在 index 清單**;且它**確實不在 go-live 樹裡** |
| `11-*` | index / 工作樹各自含不含 `^\s*(def\|class)\s`,逐格指定;兩份不得相同 |
| `12-*` | 帳本有 `ticket-declared` 且 `declared_in` 是票檔路徑;`ticket_id` 為 `null`(否則前一格先豁免,#12 不可達);票在不在 HEAD、HEAD 那版宣告不宣告該模組,逐格指定 |
| `13-*` | **行尾正規化後的雜湊**:index / 工作樹各自等不等於上游物件,逐格指定;不同向時兩者不得相同 |

**共同判準**:**該進 index 的驗在 index,不該進的驗不在。**
⚠ **前提不成立不是失敗,是「未完成」** —— 它說的是「這個案例沒造出來」,
**不是「閘門放行了」**。把兩者混在一起,下一個人會去修一個沒有壞的東西。
...
### 9.1 條文位置(2026-09-24 登記)

**條文現在在 `docs/agents/commander.md` §六〈只有 Jeff 能做的〉底下的
〈REAL 層材料的來源限制〉。**

> **為什麼放那裡而不是本文**:這條規則**不只管權威層端對端** ——
> 任何一次 REAL 層驗收都適用。寫在本文的話,
> 下一個做**別種** REAL 驗收的人找不到它;
> 而 `commander.md` §六 本來就是「只有 Jeff 能做的」那份**枚舉清單**,
> 這條正好是其中一項。它也已經是 `copy`(隨安裝出貨)且本來就點名 Jeff,
> 所以加這一項**沒有引入新的可攜性問題**。
> 本文保留**操作細節**(怎麼跑、怎麼判讀),條文歸那裡。

**來源逐字**(**不得改寫**):

> **本輪裁決確認;裁決者轉述此要求源於 2026-09-03,
> 本次未取得該日原始裁決文件。**

⚠ **本輪才發現 repo 裡查無此條。** 全庫已查到的只有兩處,
**兩處都沒有寫來源規則**:
...
⇒ **它從 2026-09-24 起是條文;在那之前它只存在於裁決端的紀錄裡。**
**不得**把它引述成一條已查證的歷史條文。

### 9.2 🔴 報告不得自己捏造執行者 —— 2026-09-24 的更正

第一版的報告範本**無論怎麼跑都印**「本輪由**裁決者本人**在普通終端機執行」。
在煙霧模式下那是**假陳述**(跑的是 agent);
而在正式模式下它也只是「沒有帶 `--smoke`」這個事實 ——
**推不出是誰按的鍵**。

> ### **旗標不是身分。**
> **一份自己捏造執行者的報告,事後與真的執行紀錄長得一模一樣** ——
> 而 REAL 層的全部價值就建立在「這份材料是誰產生的」上面。

**已修,範本按模式分流:**

| 模式 | 〈執行安排〉寫什麼 |
|---|---|
| `--smoke` | **「本輪由 agent 執行。」** + 「煙霧測試,結果不是證據,不作 REAL 結案材料」 |
| 正式 + `--executor <名字>` | 「正式驗收模式。執行者:`<名字>`」+ **「這是【自述】,腳本無法驗證身分」** |
| 正式,未給 `--executor` | 「正式驗收模式;**執行者身分由本次人工執行紀錄確認**」+ 「**不得僅憑沒帶 `--smoke` 就斷言是誰跑的**」 |

`--smoke` 與 `--executor` **互斥**(同時給 ⇒ rc=2 拒跑):
煙霧測試的執行者依定義是 agent,填一個人名進去等於把「非證據」那一格洗掉。

**兩份既有的煙霧報告已補更正註**,而且是**加註不覆寫** ——
原句、摘要表、逐案例證據、清理紀錄全部保留:

- `.dev/reports/2026-09-24T004414Z-…-SMOKE-not-evidence.md`(原始失敗那一輪)
- `.dev/reports/2026-09-24T032339Z-…-SMOKE-not-evidence.md`(補前提斷言後的重跑)

> **覆寫成正確的句子會讓一份錯的紀錄變成一份看不出改過的紀錄** ——
> 那比留著錯句加更正框糟。(`F-036` 的同一條:舊狀態不刪。)
...
| 欄 | 是什麼 |
|---|---|
| **腳本版本** | `scripts/e2e_authority_layer.py` 這支 `.py` 的 `sha256` —— 它是從**正式 repo 的工作樹**跑的 |
| **釘住的 SHA**(`--sha`) | clone 出來**受檢**的那個 commit |

**兩者可以不同步**,而混講會讓人以為「跑的版本」就是「被驗的版本」。

### 9.4 agent 自跑的定位:**診斷材料,不是證據**

`--smoke` 那個模式存在的理由就是這一條:
agent 可以自己跑,用來**排除腳本自身的機械問題**
(環境假設、防誤用誤擋、**fixture 構造沒成立**)。

- 報告檔名帶 `-SMOKE-not-evidence`,畫面與內文都標明**結果不是證據**;
- **結案只認裁決者本人執行的那一輪**;
- 煙霧測試撞到【失敗】或【意外提交成功】⇒ **立刻停,不修、不調預期,原樣回報**。
